"""Tools for setting up simulations of (updated) glycine in octanol/water/ethanol mixtures."""
...
def write_topol_file(sim_dir: PathLike, template_dir: PathLike, title: str, n_owe: list, n_gly: int):
    ...
    n_oct, n_wat, n_eth = n_owe
    if n_gly != 0:
        includes += "\n#include \"stf2/glycine.itp\""
        molecules += f"{"GCN":^13}{n_gly:^9}\n"
    if n_oct != 0:
        includes += "\n#include \"stf2/octanol.itp\""
        molecules += f"{"OCT":^13}{n_oct:^9}\n"
    if n_eth != 0:
        includes += "\n#include \"stf2/ethanol.itp\""
        molecules += f"{"ETH":^13}{n_eth:^9}\n"
    if n_wat != 0:
        includes += "\n#include \"stf2/water.itp\""
